...
function setupDropdown(allHeaders) {
  ...
  const raters = allHeaders.slice(7,16);
...
async function renderAves(data, logData) {
  ...
  const currentRaters = currentHeaders.slice(7,16); 
...
function renderCards(data) {
  ...
  const originalEight = ["Paddy", "Chris", "John", "Luke", "Tomos", "Clara", "Katie", "Eliott"];
  ...
  // --- NEW FILTER LOGIC START ---
    // This keeps albums if:
    // A) The Chooser is in your list
    // B) The Chooser field is empty/blank
    processedData = processedData.filter(item => {
        const chooser = item.Chooser ? item.Chooser.trim() : "";
        return chooser === "" || originalEight.includes(chooser);
    });
  
   if (albumCountEl) albumCountEl.innerText = `${processedData.length} ALBUMS`;
    container.innerHTML = ""; 
